...
export function ScrollGallery({ items, className = '' }: ScrollGalleryProps) {
  ...
  // Track when we're actually in the gallery section
  useEffect(() => {
    const checkPosition = () => {
      ...
      // We're in the gallery when:
      // 1. Container top is at or above viewport top (we've scrolled into it)
      // 2. Container bottom hasn't passed viewport top yet (we haven't scrolled past it)
      // 3. Sticky element is actually stuck (its top is at viewport top)
      const containerTop = containerRect.top
      const containerBottom = containerRect.bottom
      const stickyTop = stickyRect.top
      const viewportTop = 0
      
      // Gallery is active when:
      // - Container top is at or above viewport (we've entered)
      // - Container bottom is still below viewport top (we haven't exited)
      // - Sticky is stuck (top is at 0)
      const hasEntered = containerTop <= viewportTop
      const hasNotExited = containerBottom > viewportTop
      const isStuck = Math.abs(stickyTop - viewportTop) < 5 // Allow small tolerance
      
      const inGallery = hasEntered && hasNotExited && isStuck
      setIsInGallery(inGallery)
    }

    // Check on scroll with requestAnimationFrame for better performance
    let rafId: number
    const handleScroll = () => {
  ...
  // Clamp scroll progress to only update when in gallery
  const clampedProgress = useTransform(scrollYProgress, (progress) => {
    if (!isInGallery) return 0
    // Only allow progress when we're actually in the gallery
  ...
  const [debugContainerTop, setDebugContainerTop] = useState(0)
  const [debugStickyTop, setDebugStickyTop] = useState(0)
  ...
  useEffect(() => {
    ...
    const updatePositions = () => {
      if (containerRef.current && stickyRef.current) {
        setDebugContainerTop(containerRef.current.getBoundingClientRect().top)
        setDebugStickyTop(stickyRef.current.getBoundingClientRect().top)
      }
    }
  ...
  return (
    <div className={`relative w-full ${className}`}>
      {/* Scroll container - creates scroll space for tracking */}
      <div 
        ref={containerRef} 
        style={{ 
      ...
      {/* Sticky container - overlays scroll space, locks viewport */}
      <div 
        ref={stickyRef}
        className="sticky top-0 h-screen w-full overflow-hidden bg-black z-[100]"
        style={{ 
      ...
      >
        ...
        <div className="fixed top-20 right-4 bg-black/80 text-white p-4 rounded-lg z-[200] text-xs font-mono pointer-events-none">
          ...
          <div>In Gallery: {isInGallery ? 'Yes' : 'No'}</div>
          <div>Container Top: {debugContainerTop.toFixed(0)}px</div>
          <div>Sticky Top: {debugStickyTop.toFixed(0)}px</div>
        </div>
